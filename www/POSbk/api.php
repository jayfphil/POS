...
if(isset($_POST)) { 
    ...
    if(isset($_POST['cashtend'])) {
        ...
        // echo "</pre>";
        // exit();
        PDO_Execute("INSERT INTO `transaction_tb` (user_id,cash_tender,total_amt,customer_name,discount_all,total_amtdisc) VALUES(?,?,?,?,?,?)",array($_POST['userid'],$_POST['cashtend'],$_POST['totalamt'],$_POST['customer_name'],$_POST['discount_all'],$_POST['amountdue']));
        ...
        foreach ($_POST['products'] as $product => $values) {
            ...
            PDO_Execute("INSERT INTO `transactionitems_tb` (transact_id,product_id,price,quantity,sugar_level,discount) VALUES(?,?,?,?,?,?)",array($last,$values['identifier'],$values['price'],1,$values['sugar_level'],$values['discount']));
    ...
    if(isset($_POST['process_id']) && isset($_POST['process_value']) && isset($_POST['process_type'])) {
        ...
        if($_POST['process_type']=="addons-tb") {
            $table="`addons_tb`";
            $field="`at_deleted`";
        }
