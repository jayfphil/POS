    ...
    if(isset($_POST['saved'])) {
        ...
        if($result_existmonthlyreport) {
            ...
            for ($i = 1; $i <= $d; $i++) {
                  ...
                  $result_productoutput = PDO_FetchRow("SELECT SUM(`total_amt`) as 'total_amt',SUM(`cash_tender`) as 'cash_tender' FROM `transaction_tb` WHERE strftime('%Y', date_created) = '".$y."' AND strftime('%m', date_created) = '".$m."' AND strftime('%d', date_created) = '".$i."' AND `tt_voided` IS NULL");

                  $temp_income=(($result_productoutput['total_amt']) ? $result_productoutput['total_amt']: 0);
        ...
        <?php
          for ($months = 1; $months <= 12; $months++) {
              ...
              if($months==$m) {
                  ...
                  echo "<th><a href='monthly_report_summary.php?pickmonth=".$months."'>".date("F", mktime(0, 0, 0, $months))."</a></th>";
              }
          }
        ?>
        </tr>
  ...
  <div class="row">
    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
      <div class="col text-center">
        ...
        <button type="button" class="btn btn-info">Download Image</button>
      </div>
    </div>
  </div>
...
function calc_expenses() {
    ...
    $('#total_ops_expenses').val(sum.toFixed(2));
    calc_income();
}
...
function calc_monthlysales() {
    ...
    $('#grand_total').val(sum_netincome.toFixed(2));
    $('#total_bank_deposits').val(sum_bank.toFixed(2));
    calc_income();
}
