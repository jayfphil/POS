...
// echo $timeZone->getName();
    ...
    $result_dailytotal = PDO_FetchRow("SELECT SUM(total_amt) as DailyTotal FROM `transaction_tb` WHERE (`tt_reported` IS NULL AND `tt_completed` IS NOT NULL) AND DATE(`date_created`) <=  '".$date_format."'");

    $result_soldcups = PDO_FetchRow("SELECT SUM(a.quantity) as SoldCups FROM `transactionitems_tb` a INNER JOIN `transaction_tb` c ON a.`transact_id`=c.`id` WHERE (c.`tt_reported` IS NULL AND c.`tt_completed` IS NOT NULL) AND DATE(a.`date_created`) <=  '".$date_format."'");

    $result_branch = PDO_FetchRow("SELECT * FROM `branch_tb` LIMIT 1");

    $result_items = PDO_FetchAll("SELECT a.`transact_id`, a.`date_created`, c.`tt_reported`, c.`tt_completed`, a.`price`, a.`discount`, a.`addons_metajson`, b.`report_category` FROM `transactionitems_tb` a INNER JOIN `product_tb` b ON a.`product_id`=b.`id` INNER JOIN `transaction_tb` c ON a.`transact_id`=c.`id` WHERE (c.`tt_reported` IS NULL AND c.`tt_completed` IS NOT NULL) AND DATE(a.`date_created`) <=  '".$date_format."'");
    ...
    $result_existdailyreport = PDO_FetchRow("SELECT * FROM `sales_management` WHERE DATE(`date_created`) = '".$date_format."'");
    ...
    if(isset($_POST['save']) || isset($_POST['submit'])) {
        ...
        if(isset($_POST['submit'])) {
            ...
            PDO_Execute("INSERT INTO `auditlog_tb` (product_id,product_name,count_beforechange,count_afterchange,log_sources) VALUES(?,?,?,?,?)",array($result_cups['id'],$result_cups['product_name'],$result_cups['quantity'],$_POST['total_cups'],"Updated from Cashier Report"));
        }
        
        if($result_existdailyreport) {
            PDO_Execute("UPDATE `sales_management` SET ".implode(", ",$update)." WHERE DATE(`date_created`) = '".$date_format."'");
        } else {
...
<?php
        if(isset($_POST['save'])) {
            echo '<script type="text/javascript">','swal("Report successfully saved!");','</script>';
        }
        if(isset($_POST['submit'])) {
            echo '<script type="text/javascript">','swal("Report successfully generated!");','</script>';
