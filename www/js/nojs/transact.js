(function ($) {
 "use strict";
  ...
  $(document).ready(function() {
    ...
    $(document).on('blur', '.editable_sale', function (e) {

        var editpricerow = $(this).attr("data-editable");
        var origprice = parseFloat($('#hiddeneditable_'+editpricerow).attr("data-value"));
        
        var amountdue = parseFloat($("#amountdue").text()); // gets the latest then added with discount
        var totalamt = parseFloat($("#totalamt").text()); // gets the latest then added with discount

        var neweditamountdue = customFixed((amountdue-origprice)+parseFloat($(this).val()));
        var newedittotalamt = customFixed((totalamt-origprice)+parseFloat($(this).val()));

        $("input[name*='products["+editpricerow+"][price]'").val($(this).val());

        $("#amountdue").text(customFixed(neweditamountdue));
        $("#totalamt").text(customFixed(newedittotalamt));
    ...
    function auto_clone(content,datatable){
        ...
        $.ajax({
            ...
            success: function (result) {
              ...
              if(result.products.pt_customprice>0) {
                var editable_price = "<input type='text' class='editable_sale' id='editableexist_"+tb_row+"' data-editable='"+tb_row+"' value='"+customFixed($clone.attr("data-price"))+"'>";
                var editable_discount ="<span id='hiddeneditable_"+tb_row+"' data-value='"+customFixed($clone.attr("data-price"))+"'><b>"+0+"%</b></span>";
              } else {
                var editable_price = "<span id='discount_"+tb_row+"'>"+customFixed($clone.attr("data-price"))+"</span>";
                var editable_discount ="<a href='#' class='discitem' data-toggle='modal' data-target='#discounts_modal' data-discrow='"+tb_row+"' data-name='"+$clone.attr("data-name")+"'><b>"+0+"%</b></a>";
              }
    ...
    // MutationObserver binding is not effective

    $('#amountdue').bind("DOMSubtreeModified",function(){
      ...
      // if(discount_allval>0) {
        ...
        for (var i = 0; i <= $("#tablerow").val(); i++) {
          ...
          if($("input[name*='products["+i+"][price]'").length > 0) {
               ...
               // if($("#editableexist_"+i).length>0 && $("#editableexist_"+i).val()) {
               //     var pricey = parseFloat($("#editableexist_"+i).val());
               // } else {
